...
uint32_t AudioPolicyManager::getDeviceForStrategy(routing_strategy strategy, bool fromCache)
{
    ...
    case STRATEGY_PHONE:
        ...
        default:    // FORCE_NONE
            ...
            if (mPhoneState != android_audio_legacy::AudioSystem::MODE_IN_CALL) {
                ...
                if (mForceUse[android_audio_legacy::AudioSystem::FOR_DOCK] == android_audio_legacy::AudioSystem::FORCE_BT_CAR_DOCK) {
                    device = mAvailableOutputDevices & android_audio_legacy::AudioSystem::DEVICE_OUT_BLUETOOTH_A2DP;
                } else if (mForceUse[android_audio_legacy::AudioSystem::FOR_DOCK] == android_audio_legacy::AudioSystem::FORCE_BT_DESK_DOCK) {
    ...
    case STRATEGY_SONIFICATION:
        ...
        if (mForceUse[android_audio_legacy::AudioSystem::FOR_DOCK] != android_audio_legacy::AudioSystem::FORCE_BT_CAR_DOCK) {
            ...
            if (mForceUse[android_audio_legacy::AudioSystem::FOR_DOCK] == android_audio_legacy::AudioSystem::FORCE_BT_DESK_DOCK) {
                if (mAvailableOutputDevices & android_audio_legacy::AudioSystem::DEVICE_OUT_WIRED_HEADPHONE) {
                    device |= android_audio_legacy::AudioSystem::DEVICE_OUT_WIRED_HEADPHONE;
                } else if (mAvailableOutputDevices & android_audio_legacy::AudioSystem::DEVICE_OUT_WIRED_HEADSET) {
    ...
    case STRATEGY_MEDIA: {
        uint32_t device2 = mAvailableOutputDevices & android_audio_legacy::AudioSystem::DEVICE_OUT_AUX_DIGITAL;
#ifdef WITH_A2DP
        if (mA2dpOutput != 0) {
            if (device2 == 0) {
                ...
                if (mForceUse[android_audio_legacy::AudioSystem::FOR_DOCK] == android_audio_legacy::AudioSystem::FORCE_BT_CAR_DOCK &&
                    ...
                    mPhoneState == android_audio_legacy::AudioSystem::MODE_RINGTONE) {
                    device2 = mAvailableOutputDevices &
                              (android_audio_legacy::AudioSystem::DEVICE_OUT_SPEAKER |
                               android_audio_legacy::AudioSystem::DEVICE_OUT_WIRED_HEADPHONE |
                               android_audio_legacy::AudioSystem::DEVICE_OUT_WIRED_HEADSET);
                }
            }
        }
#endif
        ...
        // output routing
        if (mPhoneState == android_audio_legacy::AudioSystem::MODE_IN_CALL &&
            !android_audio_legacy::AudioSystem::isA2dpDevice((android_audio_legacy::AudioSystem::audio_devices)device) &&
            device != getDeviceForStrategy(STRATEGY_PHONE, false)) {
            device = 0;
...
float AudioPolicyManager::computeVolume(int stream, int index, audio_io_handle_t output, uint32_t device)
{
    ...
    if (output == mA2dpOutput &&
        mForceUse[android_audio_legacy::AudioSystem::FOR_DOCK] == android_audio_legacy::AudioSystem::FORCE_BT_CAR_DOCK) {
        return 1.0;
    }
#endif
    ...
    // avoid saturation
    if (mPhoneState == android_audio_legacy::AudioSystem::MODE_IN_CALL && android_audio_legacy::AudioSystem::isBluetoothScoDevice((android_audio_legacy::AudioSystem::audio_devices)device)) {
    ...
    if ((mForceUse[android_audio_legacy::AudioSystem::FOR_DOCK] == android_audio_legacy::AudioSystem::FORCE_BT_CAR_DOCK) &&
            (stream == android_audio_legacy::AudioSystem::MUSIC) &&
            (device & (android_audio_legacy::AudioSystem::DEVICE_OUT_WIRED_HEADPHONE |
                android_audio_legacy::AudioSystem::DEVICE_OUT_WIRED_HEADSET))) {
